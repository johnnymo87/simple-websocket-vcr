...
module WebSocketVCR
  ...
  class RecordableFayeWebsocketClient
    ...
    def _write(data)
      if @live
        @client.__send__(:send, data)
        @session.store(operation: 'write', event: :send, data: data)
      else
    ...
    def _read(event, &_block)
      if @live
        rec = @session
        @client.on(event) do |msg|
          params = { operation: 'read', event: event }
          if msg.respond_to?(:data)
            data = msg.type.to_s == 'text' ? msg.data : Base64.encode64(msg.data.pack("C*"))
    ...
    def parse_data(record)
      if (data = record.data)
        data = Base64.decode64(data).unpack("C*") if record.type != 'text'
